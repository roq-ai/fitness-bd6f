...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBookings({
        relations: ['customer', 'trainer', 'gym'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['workout_type.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'workout_type', header: 'Workout Type', accessorKey: 'workout_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/customers/view/${record.customer?.id}`}>
              {record.customer?.membership_start_date}
            </Link>
          ),
        }
      : null,
